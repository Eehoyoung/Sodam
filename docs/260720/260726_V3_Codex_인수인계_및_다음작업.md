...
**2026-07-26 세션 후기(2026-07-27~28)**: §9.1·§9.2에서 다룬 변경사항 전부 실제로 커밋·**푸시 완료**됨(`d0a607d..773af4a`, 14개 커밋, 기능/화면별 분리). GlobalExceptionHandler.java는 세션보안(423/429)과 낙관적락(409) 훅이 섞여있어 훅 단위로 임시 분리 후 각각 다른 커밋에 반영. 상세 커밋 목록은 git log 참조.

### 9.3 v3 시각 하네스 실제 캡처 착수 — 예상보다 훨씬 큰 잔여 범위 발견 (2026-07-28)

P1(WP-09) 이어서 진행하려고 실제 Android 에뮬레이터(`emulator-5554`)로 캡처를 시작했는데, **자동화 문제가 아니라 범위 문제**였다.

- **딥링크 형식 정정**: `capture-v3-android-visual.ps1`이 만드는 URI(`sodam://v3/$Source/$ScreenId`)는 실제로 작동하지 않는다 — react-navigation의 linking 파서가 `v3`를 host로 해석해버려 라우팅이 안 됨. adb로 직접 검증한 결과 **`sodam:///v3/{source}/{screenId}`(슬래시 3개, host 명시적 empty)** 형식이어야 실제로 네비게이션된다. 스크립트 자체가 고쳐지지 않은 채였던 것으로 보임 — 다음에 이 스크립트를 손댈 때 반영할 것.
- **핵심 발견**: 154개 카탈로그 화면 중 `V3VisualHarnessScreen.tsx`의 디스패치 체인에 실제로 배선·캡처된 화면은 **71개**였다(최초 grep으로 "41개"라고 잘못 셌던 걸 `npm run visual:v3:catalog`의 authoritative summary로 정정 — `mapped:154, nativeCapturePairs:71, strictPassed:70`). 나머지는 `screenId`가 어떤 `V3_VISUAL_SCREEN_IDS.*`/`*StateIds` 맵에도 안 걸려 "미배선 정본 화면"(`accessibilityLabel="미배선 v3 시각 정본"`) 플레이스홀더로 떨어진다. 이 플레이스홀더는 캡처 스크립트가 찾는 `v3-visual-{source}-{screenId}` 마커를 안 갖고 있어 15초씩 폴링하다 항상 실패로 기록된다 — 배치 캡처 중 42번째 화면부터 78개 연속 실패했던 게 에뮬레이터/Metro 장애가 아니라 정확히 이 미배선 구간을 지나가고 있었기 때문이었다.
- **실제 캡처 진행**: 새로 작성한 `frontend/scripts/v3-visual-capture-all.ps1`(전체 카탈로그 순회, 이미 있는 파일 스킵, 화면 하나 실패해도 계속 진행)로 실행 가능한 만큼 진행. `npm run visual:v3:compare:native`(native-reference vs actual, 0/0 엄격 기준) 최종 결과: **154개 중 70개 pixel-perfect 통과(45%), 83개 missing-reference(미배선이라 애초에 못 찍음), 1개 진짜 pixel-mismatch(`sodam-v3-03-employee--020`, differingPixels 372730 — 원인 미확인, 확인 필요)**.
- **정정**: HTML 캐논likal(`docs/260720/artifacts/*.html`) 대 실제 앱(`actual`) 직접 비교(`npm run visual:v3:compare`, reference=HTML)는 채널 threshold 0/pixel budget 0 기준으로는 **의미가 없다** — 브라우저 렌더러와 네이티브 렌더러의 폰트·안티에일리어싱 차이만으로도 수십만 픽셀이 달라진다(실측: 41개 화면 전부 50만 픽셀 안팎 차이). 진짜 의미있는 엄격 비교는 **native-reference(같은 앱이 그린 정본) vs actual(같은 앱의 실제 화면)** 조합(`visual:v3:compare:native`)뿐이다 — 둘 다 같은 렌더러를 쓰므로 0/0이 현실적인 기준이 된다.
- **잔여 작업 재정의**: "153장 더 캡처"가 아니라 **"83개 화면 각각에 대해 네이티브 재현(NativeReference*) 컴포넌트를 새로 작성해 디스패치 체인에 배선"** 하는 것이 진짜 남은 일이다 — 이미 배선된 41개를 만드는 데 든 노력과 비슷하거나 그 이상 규모의 프론트엔드 코딩 작업으로 추정됨. 캡처 자동화 자체(딥링크·uiautomator·크롭·픽셀대조 파이프라인)는 완전히 검증됐고 문제없다.
- 이 세션 작업(스크립트 신규 파일 `v3-visual-capture-all.ps1`, `artifacts/v3-visual/` 캡처 산출물)은 **미커밋** 상태로 남겨둠(산출물은 어차피 커밋 대상 아님, 스크립트는 사용자 확인 후 커밋 예정).

### 9.4 83화면 배선 작업 착수 — 인프라 버그 2건 수정 (2026-07-28)

§9.3에서 정의한 진짜 잔여 작업(83개 화면에 네이티브 재현 컴포넌트 작성+배선)을 순차 진행하며, 첫 몇 화면에서 캡처 자체가 막히는 **인프라 버그 2건**을 발견·수정했다. 둘 다 이후 83개 전체에 영향을 주는 근본 수정이라 여기 남긴다.

1. **딥링크 슬래시 버그(재확인·수정 완료)**: `frontend/scripts/capture-v3-android-visual.ps1`의 `$uri = "sodam://v3/$Source/$ScreenId"`가 실제로 작동하지 않는 원인이었던 걸 §9.3에서 발견은 했으나 스크립트 자체는 안 고쳐져 있었다. 이번에 `sodam:///v3/$Source/$ScreenId`(슬래시 3개)로 실제 수정 완료.
2. **Modal 기반 시트의 captureMarker 미전달 버그(신규 발견·수정)**: `BottomSheet.tsx`는 `captureMarker?: string` prop을 받아 시트 본문 안에 비가시 텍스트로 렌더링하는 기능이 이미 있었다(RN `Modal`은 별도 Android 창으로 뜨기 때문에, 배경 화면에 있는 `VisualRouteFrame`의 마커 텍스트는 uiautomator가 Modal 창을 덤프할 때 안 잡힌다 — 이미 `PushPrimerSheet.tsx`가 이 패턴을 쓰고 있었음). 그런데 `AttendanceSheets.tsx`의 `CheckoutConfirmSheet`/`BreakTimerSheet`/`ManualRecordSheet`/`PersonalRecordEditSheet` 4개는 이 `captureMarker`를 안 받고 있어서 캡처가 항상 15초 타임아웃으로 실패했다. 4개 전부에 `captureMarker?: string` prop을 추가해 `BottomSheet`로 그대로 전달하도록 수정(순수 추가적 변경, 기존 실제 앱 동작에는 영향 없음 — 값이 없으면 기존과 동일).

**83개 진행 현황**: 진행 체크리스트 `docs/260720/v3-visual-83-progress.md` 참조, 완료할 때마다 체크 표시. 각 화면은 (a) 캐논니컬 HTML 카드 대조 → (b) 실 화면 파일에 `visualFixture` prop 필요 시 추가(순수 옵셔널, 기존 실사용 경로 무영향) → (c) 독립적인 `NativeReference*` 컴포넌트 작성 → (d) 디스패치 배선 → (e) 에뮬레이터 실캡처 + `visual:v3:compare:native`(0/0 엄격) 통과까지 확인하는 순서로 처리한다. 완료 예시(080 PersonalRecordEdit)에서 실제로 픽셀 불일치를 잡은 사례: 하네스 파일 안에 이름이 같은 서로 다른 `styles.form`(마진/gap 값이 다름)이 있어 새 컴포넌트가 무관한 스타일을 참조하고 있었다 — 화면마다 이런 종류의 실수가 나올 수 있으므로 매 화면 실캡처 검증이 필수다.
