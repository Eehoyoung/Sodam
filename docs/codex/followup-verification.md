...
### 시안 정합 후속 (2026-06-29, 추가)
스케줄 목업(`schedule-feature-mockup.html`) 대비 미구현분 중 고가치·자기완결 항목 구현:
- **저장 전 비차단 경고**: 중복 근무(같은 직원·같은 날 시간겹침, 수정 시 자기 제외)·영업시간 밖(운영시간 GET 대비)·휴게 누락(근로기준법 §54, 4h/8h). 라이브 패널 표시, 저장은 허용. `storeService.getStoreOperatingHours` 추가.
- **직원별 보기 탭**: 목록/직원별/보드 3토글. 직원 그룹별 시프트(건수·총시간).
- ✅ 에뮬레이터 검증: 휴게 §54 경고·중복 경고(수정 진입 시 동일날 겹침)·직원별 그룹(김알바 4건·31.0h) 렌더 확인.
- ✅ **템플릿 탭 구현·검증 완료(2026-06-29)**: 사장 확정 설계(주간패턴·매장별·직원고정+스킵·전티어)로 BE(엔티티+V24+서비스+컨트롤러+테스트6건)·FE(템플릿 탭) 구축. 설계문서 `shift-template-design.md`. 에뮬 E2E: 저장→목록→적용(이번주 4→8건·31→62h 갱신)·API 적용 요일매핑(월야간→월·익일보존, 화→화, 목→목)·비활성 스킵 보고 확인.
- ⏸ 미구현(잔여): 바텀시트형 근무 추가(현 인라인 폼으로 동작), 시간 프리셋(직원 없는 블록) 탭.
