...
- **소담 포지셔닝 전환(근태앱 → 노무 리스크 사전예측)** — 계획서 `docs/260815/260815_소담_포지셔닝전환_계획서.md`,
  작업지시 `docs/260815/260815_소담_포지셔닝전환_goal.json`, 코드 작업지침
  `docs/260815/260815_소담_코드작업_지침.md`. WP-1~WP-9 전부 구현·테스트·문서 반영 완료
  (커밋 히스토리 참조). **후속 작업은 `docs/260816/260816_포지셔닝전환_후속작업_계획서.md`** —
  가장 급한 것은 WP-A(상시근로자 시뮬레이션 FE 배선, `V-6`) — BE API는 완성됐지만 FE 소비처가
  없어 데모 4단계가 지금 재현 불가능하다. 그 외 사람 전용 검증(`V-4`·`V-5`, `V-6` 선행 필요),
  신설 게이트 `G-16~G-19`(노무사·법무 서면 회신), `H-6`(LLM 실키 활성화)·`H-7`(구독료 인상
  실행) 인간 승인. `D-7`(labor-health 활용 방향)은 2026-08-16 결정(안 B: 별도 상세 분석
  화면 신설)으로 해소돼 WP-B로 편입됨. 이 영역 작업 요청을 받으면
  먼저 `docs/RELEASE_GATES.md`의 해당 게이트 절을 읽는다.
