 ...
 *   - 사장: owner@sodam.dev / sodam1234 (MASTER, BUSINESS 구독)
 *   - 직원: staff@sodam.dev / sodam1234 (EMPLOYEE)
 *   - 매장: 소담 데모 매장 (서울 중구 좌표, 반경 500m, 시급 12,000)
 *
 * 비밀번호 단순(약함)함은 의도적임 — 개발 편의. 운영(prod)에서는 절대 실행 안 됨.
 */
@Slf4j
@Component
@Profile("dev")
...
public class DevSeedRunner implements CommandLineRunner {
    ...
    public void run(String... args) {
        ...
        log.info("DevSeed: 완료 — owner=owner@sodam.dev / staff=staff@sodam.dev (비번 모두 sodam1234), 매장 #{}",
                store.getId());
    }
    ...
    private User buildUser(String email, String name, UserGrade grade) {
        ...
        u.setPassword(passwordEncoder.encode("sodam1234"));
